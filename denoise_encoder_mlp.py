...
class NoisyTransformerEncoderBlock(nn.Module):
    def __init__(self, embedding_dim, nhead=8, dim_feedforward=2048):
        super().__init__()
    ...
    def forward(self, x_feat, z_prev, alpha_bar_t=None):
        """
        x_feat: [B, L, D]
        z_prev: [B, D] or [D] (latent vector)
        alpha_bar_t: scalar or [B] tensor
        Returns: z_out: [B, D]
        """
        # Handle singleton batch: z_prev [D] → [1, D]
        if z_prev.dim() == 1:
            z_prev = z_prev.unsqueeze(0)  # [1, D]

        B, D = z_prev.shape

        # Handle alpha_bar_t shape (broadcast-safe)
        if alpha_bar_t is not None:
            if alpha_bar_t.dim() == 0:
                alpha_bar_t = alpha_bar_t.expand(B).unsqueeze(1)  # [B, 1]
        ...
        # Expand z for attention
        z = z_noisy.unsqueeze(1)  # [B, 1, D]
        z_residual = z.clone()

        # Self-attention: latent attends to x_feat
        z_attn, _ = self.self_attn(z, x_feat, x_feat)  # [B, 1, D]
        z = self.norm1(z + z_attn)

        # Denoising feedforward
        z_denoised = self.denoiser(z.squeeze(1)).unsqueeze(1)  # [B, 1, D]
        z = self.norm2(z + z_denoised)

        # Gated residual
        gate = torch.sigmoid(self.gate)
        z = gate * z + (1 - gate) * z_residual  # [B, 1, D]

        return z.squeeze(1)  # [B, D]


class NoPropDTEncoder(nn.Module):
    def __init__(self, num_classes, embedding_dim, nhead = 8, T=6, eta=0.03, dropout=0.1, dim_feedforward=2048):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            NoisyTransformerEncoderBlock(embedding_dim,
                                          nhead=nhead,
...
def train_noprop_encoder(model, train_loader, test_loader, epochs, lr, weight_decay, best_metric='f1'):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for t in range(model.T):
            for x, y in train_loader:
                ...
                # Step 1: Generate clean target uy
                with torch.no_grad():
                    uy = model.attn_pool(x)  # [B, D]

                # Step 2: Inject noise (cosine schedule)
                alpha_bar_t = model.alpha_bar[t]
                if alpha_bar_t.dim() == 0:
                    alpha_bar_t = alpha_bar_t.expand(B).unsqueeze(1)  # [B, 1]

                noise = torch.randn_like(uy)
                z_t = torch.sqrt(alpha_bar_t) * uy + torch.sqrt(1 - alpha_bar_t) * noise  # [B, D]

                # Step 3: Denoising step
                z_pred = model.blocks[t](x, z_t, alpha_bar_t=alpha_bar_t)  # [B, D]

                # Step 4: Denoising loss
                loss_l2 = F.mse_loss(z_pred, uy)
                loss = 0.5 * model.eta * model.snr_diff[t] * loss_l2

                # Step 5: Final step includes classification
                if t == model.T - 1:
                    logits = model.classify(z_pred)  # [B, C]
...
def run():
    ...
    train_noprop_encoder(no_prob_model, train_loader, test_loader, epochs=epochs, lr=lr, weight_decay=weight_decay, best_metric='f1')

    print_test_preds(no_prob_model, test_loader)


    save_path = "nopropdt_encoder.pth"  # or /dbfs/tmp/...
